...
def clean_data(csv_file):
    '''
    return a dataframe which contains cleaned data of one network-stats record.
    
    :param: csv_file: a raw csv_file generated by network-stats
    '''
    temp=csv_file[['Time','1->2Bytes','2->1Bytes','1->2Pkts','2->1Pkts']]
    temp=temp.groupby('Time').sum()
    outbyte=[]
    inbyte=[]
    outpkts=[]
    inpkts=[]
    index=range(temp.index[0],temp.index[-1]+1)
    counter=0
    for i in index:
        if i in temp.index:
            outbyte.append(temp['1->2Bytes'][i])
            inbyte.append(temp['2->1Bytes'][i])
            outpkts.append(temp['1->2Pkts'][i])
            inpkts.append(temp['2->1Pkts'][i])
            counter+=1
        else:
            outbyte.append(0)
            inbyte.append(0)
            ...
            inpkts.append(0)
            counter+=1
    result=pd.DataFrame(data={'1->2Bytes':outbyte,'2->1Bytes':inbyte,'1->2Pkts':outpkts,'2->1Pkts':inpkts},index=index)
    return result
